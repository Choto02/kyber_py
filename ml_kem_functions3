...
def K_PKE_Encrypt(ek_pke,m,r):
    ...
    i = 0
    for i in range (KYBER_K):
        r_bold[i] = SamplePolyCBD(prf(ETA1,r,bytes([N])),ETA1)
        N = N + 1

    i = 0
    for i in range (KYBER_K):
        e1[i] = SamplePolyCBD(prf(ETA2,r,bytes([N])),ETA2)
        N = N + 1
...
if __name__ == "__main__":
    ...
    ciphertext = K_PKE_Encrypt(private_key, bytes(12345),bytes(101010))
    print("Ciphertext:", ciphertext)

    #ciphertext = bytes(999)
    #K_PKE_Decrypt(private_key, ciphertext)
