...
def Decaps_internal(dk, c):
    ...
    """
    if len(c) != 32 * (du * KYBER_K + dv):
        raise ValueError(
            f"ciphertext type check failed. Expected {32 * (du * KYBER_K + dv)} bytes and obtained {len(c)}"
        )
    if len(dk) != 768 * KYBER_K + 96:
        raise ValueError(
            f"decapsulation type check failed. Expected {768 * KYBER_K + 96} bytes and obtained {len(dk)}"
        )

    # Parse out data from dk
    dk_pke = dk[0 : 384 * KYBER_K]
    ek_pke = dk[384 * KYBER_K : 768 * KYBER_K + 32]
    h = dk[768 * KYBER_K + 32 : 768 * KYBER_K + 64]
    z = dk[768 * KYBER_K + 64 :]
...
if __name__ == "__main__":
    ...
    #TESTING COMPLETE KEM
    ek, dk = KeyGen()
    K, c = Encaps(ek)
    print("Encapsulated IS: ", K)
    K_prime = Decaps(dk, c)
    print("Decapsulated IS: ", K_prime)
